# 7-21 求特殊方程的正整数解（Perl语言实现）

## 前言

本题（7-21 求特殊方程的正整数解）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的perl代码进行讲解。
...
for my $X (1 .. int(sqrt($N))) {
    for my $Y ($X .. int(sqrt($N))) {
        if ($X * $X + $Y * $Y == $N) {
            print "$X $Y\n";
            $found = 1;
        }
...
1. **变量声明**：使用my关键字声明$N存储输入值，$found标记是否找到解（初始为0）
2. **输入读取**：使用chomp(my $N = <STDIN>)读取正整数N
3. **外层循环（X枚举）**：for my $X (1..int(sqrt($N)))，枚举X从1到√N
4. **内层循环（Y枚举）**：for my $Y ($X..int(sqrt($N)))，枚举Y从X到√N
5. **方程判断**：若$X*$X + $Y*$Y == $N，则使用print("$X $Y\n")输出解，设置$found=1
6. **无解判断**：双重循环结束后，若$found == 0，输出"No Solution\n"
7. **程序结束**：脚本执行完毕

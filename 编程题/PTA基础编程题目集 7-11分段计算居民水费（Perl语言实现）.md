# 7-11 分段计算居民水费（Perl语言实现）

## 前言

本题（7-11 分段计算居民水费）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的perl代码进行讲解。
...
2. **变量声明与数据输入**
   - `my $x = <STDIN>;`：声明词法变量$x并从标准输入读取一行
     - `<STDIN>`：从标准输入读取一行数据
     - `chomp $x;`：去除行尾换行符
     - $x：存储输入的月用水量（吨）
   - `my $y;`：声明词法变量$y，用于存储计算得到的应交水费（元）
...
flowchart TD
    A[开始] --> B[注释说明程序功能]
    B --> C[声明词法变量$x和$y]
    C --> D[<STDIN>读取一行存入$x<br>chomp去除换行符]
    D --> E{$x <= 15?}
    E -->|是| F[$y = 4 * $x / 3]
    E -->|否| G[$y = 2.5 * $x - 17.5]
    F --> H[printf格式化$y保留两位小数<br>输出结果并换行]
    G --> H
    H --> I[Perl脚本执行结束]
```

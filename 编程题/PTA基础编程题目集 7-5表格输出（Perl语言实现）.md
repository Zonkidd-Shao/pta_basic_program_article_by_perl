...
本题（7-5 表格输出）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的perl代码进行讲解。
...
1. **输出顶部分隔线**：使用`print`打印36个减号组成的横线，手动添加换行符`\n`。

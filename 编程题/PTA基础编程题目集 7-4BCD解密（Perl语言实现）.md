...
本题（7-4 BCD解密）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的perl代码进行讲解。
...
chomp(my $n = <STDIN>);
my $highDigit = int($n / 16);
my $lowDigit = $n % 16;
my $result = $highDigit * 10 + $lowDigit;
print "$result\n";
```
...
2. **提取高位数字**：`int($n / 16)`整除16，得到十六进制表示的高位数字（对应BCD的十位）。
3. **提取低位数字**：`$n % 16`取模16，得到十六进制表示的低位数字（对应BCD的个位）。
4. **组合正确结果**：高位数字×10 + 低位数字，得到正确的十进制数，存入标量变量`$result`。
5. **输出结果**：使用`print()`输出正确的十进制数并手动添加换行符`\n`。
6. **程序结束**：Perl脚本执行完毕，自然结束。
...
flowchart TD
  A["开始"] --> B["my $n = <STDIN>\nchomp($n)\n读取错误十进制数"]
  B --> C["my $highDigit = int($n / 16)\n提取十六进制高位（十位）"]
  C --> D["my $lowDigit = $n % 16\n提取十六进制低位（个位）"]
  D --> E["my $result = $highDigit * 10 + $lowDigit\n组合正确十进制数"]
  E --> F["print $result . '\\n'\n输出正确结果"]
  F --> G["脚本执行完毕\n程序结束"]
```

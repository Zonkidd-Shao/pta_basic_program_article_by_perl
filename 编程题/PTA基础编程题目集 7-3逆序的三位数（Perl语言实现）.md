...
本题（7-3 逆序的三位数）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的perl代码进行讲解。
...
my $tens = int($n / 10) % 10;
my $units = $n % 10;
my $reversed = $units * 100 + $tens * 10 + $hundreds;
print "$reversed\n";
```

## 代码流程说明

1. **读取输入**：使用`<STDIN>`读取一行输入，通过`chomp()`去除换行符，赋值给标量变量`$n`。
2. **声明变量**：使用`my`声明`$n`、`$hundreds`、`$tens`、`$units`、`$reversed`等标量变量。
3. **提取百位数字**：`int($n / 100)`整除运算，只保留百位及以上部分（三位数的百位）。
4. **提取十位数字**：先`$n / 10`去掉个位，再`% 10`取模得到十位数字，通过`int()`确保整数。
5. **提取个位数字**：`$n % 10`取模10直接得到个位数字。
6. **逆序重组**：个位×100 + 十位×10 + 百位，得到逆序后的新数。
7. **输出结果**：使用`print()`输出逆序数并手动添加换行符，整数输出自动省略前导零。
8. **程序结束**：Perl脚本自然结束。
...
flowchart TD
  A["开始"] --> B["$n = <STDIN>\nchomp($n)\n读取三位数"]
  B --> C["my $hundreds, $tens, $units, $reversed\n声明标量变量"]
  C --> D["$hundreds = int($n / 100)\n提取百位数字"]
  D --> E["$tens = int(($n / 10) % 10)\n提取十位数字"]
  E --> F["$units = $n % 10\n提取个位数字"]

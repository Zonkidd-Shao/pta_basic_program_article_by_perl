# 7-36 复数四则运算（Perl语言实现）

## 前言

本题（7-36 复数四则运算）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的perl代码进行讲解。
...
my ($a1, $b1, $a2, $b2) = split /\s+/, <STDIN>;

print_op($a1, $b1, '+', $a2, $b2, $a1 + $a2, $b1 + $b2);
print_op($a1, $b1, '-', $a2, $b2, $a1 - $a2, $b1 - $b2);
print_op($a1, $b1, '*', $a2, $b2, $a1 * $a2 - $b1 * $b2, $a1 * $b2 + $a2 * $b1);
my $denom = $a2 * $a2 + $b2 * $b2;
print_op($a1, $b1, '/', $a2, $b2,
         ($a1 * $a2 + $b1 * $b2) / $denom,
         ($b1 * $a2 - $a1 * $b2) / $denom);
```
...
### 1. printComplex函数
- 输入：复数的实部a、虚部b
- 功能：按规则格式化输出复数（print不自动加换行需手动\n，printf格式化）
- 流程：
  - abs()判断实部虚部都接近0 → print输出"0.0"
  - 仅实部接近0 → printf("%.1fi", $b)输出虚部+"i"
  - 仅虚部接近0 → printf("%.1f", $a)输出实部
  - 都不为0 → 虚部正用"+"连接，虚部负直接连接（自带负号）

### 2. printOp函数
- 输入：两个复数和运算符、结果复数
- 功能：输出完整运算表达式
- 流程：print输出左括号 + printComplex打印C1 + print输出右括号+运算符+左括号 + printComplex打印C2 + print输出右括号+= + printComplex打印结果 + print输出换行\n
...
```mermaid
flowchart TD
    A["开始"] --> B["<STDIN>读取一行, split分割a1 b1 a2 b2"]
    ...
    subgraph printOp流程
        M["print输出左括号"] --> N["printComplex打印C1"]
        N --> O["print输出)运算符("]
    ...
    subgraph printComplex流程
        T{"abs(a)<0.05 且 abs(b)<0.05?"} -->|是| U["print输出0.0"]
        T -->|否| V{"abs(a)<0.05?"}
        V -->|是| W["printf %.1fi, b"]
        V -->|否| X{"abs(b)<0.05?"}
        X -->|是| Y["printf %.1f, a"]
        X -->|否| Z{"b>0?"}
        Z -->|是| AA["printf %.1f+%.1fi, a, b"]
        Z -->|否| AB["printf %.1f%.1fi, a, b"]
    end
```
...
flowchart TD
    ...
    F --> G[按公式实现4种运算]
    G --> H[修复printf格式化<br>正确拼接表达式]
    H --> I[编写完整代码]
    I --> J[用样例1验证输出]
    J --> K{格式和数值正确?}
    K -->|是| L[用样例2验证边界情况]
    K -->|否| M[检查公式/printf格式/阈值]
    M --> I
    L --> N{边界正确?}
    N -->|是| O[完成]

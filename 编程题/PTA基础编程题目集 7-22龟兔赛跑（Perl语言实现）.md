...
本题（7-22 龟兔赛跑）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的perl代码进行讲解。
...
1. **输入阶段**：使用chomp(my $T = <STDIN>)读取比赛时间T
2. **变量初始化**：使用my声明$turtle/$rabbit记录双方距离（初始0），$restTime记录兔子剩余休息时间（初始0），$runTime记录兔子连续跑步时间（初始0）
3. **逐分钟模拟循环**：for my $t (1..$T)
   - 乌龟始终前进3米：$turtle = $turtle + 3
   - 判断兔子状态：若$restTime > 0则$restTime--（休息中），否则$rabbit += 9，$runTime++（跑步中）
   - 兔子跑满10分钟时（$runTime == 10）：若$rabbit > $turtle则$restTime = 30，无论是否休息$runTime都重置为0
4. **结果比较**：比较$rabbit和$turtle的最终值

...
- 时间复杂度：`O(N)`，首元素初始化后，剩余元素各比较一次。
- 空间复杂度：`O(N)`，当前实现复制了输入数组；直接通过引用访问时额外空间为 `O(1)`。

## 常见易错点

1. 最大值不能初始化为 0，否则全为负数时会得到错误结果，应使用首元素。
2. 首元素已经作为初值，比较循环应从下标 1 开始。
3. 比较应使用数值关系，不能使用字符串字典序。
4. 题目保证 N 至少为 1；空集合的最大值没有定义。

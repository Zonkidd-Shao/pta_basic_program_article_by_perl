...
- 时间复杂度：`O(n)`，霍纳法则从最高次项到常数项只遍历一次系数。
- 空间复杂度：`O(n)`，当前实现复制了系数数组；直接通过引用访问时额外空间可降为 `O(1)`。

## 常见易错点

1. `a[0]` 是常数项，`a[n]` 是最高次项，迭代必须从 `$n` 递减。
2. n 为 0 时应直接返回 `a[0]`，不能访问不存在的更高次项。
3. 霍纳迭代应执行 `$sum = $sum * $x + $a[$i]`，乘法和加法顺序不能颠倒。
4. 函数接收的是系数数组引用，使用前要正确解引用或通过引用访问。

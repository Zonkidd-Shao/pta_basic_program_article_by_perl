...
my $line = <STDIN>;
chomp $line;
my @list = $n > 0 ? split(/\s+/, $line) : ();
print FactorialSum(\@list), "\n";
```
...
设链表有 m 个结点，第 j 个结点的值为 vⱼ：

- 时间复杂度：`O(m + ∑vⱼ)`，遍历结点需要 O(m)，每个阶乘还需要与结点值同阶的累乘操作。
- 空间复杂度：`O(m)`，当前实现复制了数组；阶乘计算和总和累加只使用固定数量的变量。

## 常见易错点

1. 阶乘初值应为 1，保证 `0! = 1`。
2. 空链表的阶乘和应为 0，不能把总和初值设为 1。
3. `FactorialSum` 必须遍历所有结点，不能只计算首个结点。
4. 数组元素表示结点的 Data 值，不要把结点下标当作待计算数据。

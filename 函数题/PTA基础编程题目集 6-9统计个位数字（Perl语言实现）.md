...
- 时间复杂度：`O(d)`，每次循环处理 N 的一位数字。
- 空间复杂度：`O(1)`，只使用中间数值、当前位和计数器。

## 常见易错点

1. 负号不是数字，应先对 N 取绝对值再统计。
2. N=0 且 D=0 时返回 1；N=0 且 D 非 0 时返回 0。
3. 统计对象是整个整数，不能只检查 `N % 10` 得到的个位。
4. 反复除以 10 结束时，要确保最高位也被处理；当前 `while ($m)` 写法可以覆盖这一点。

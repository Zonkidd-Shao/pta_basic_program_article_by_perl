...
设输入为 N，结果 `N!` 的十进制位数为 d：

- 时间复杂度：需要进行 N-1 次大数乘法；若只按乘法次数计为 `O(N)`，实际耗时还会随中间结果位数增长。
- 空间复杂度：`O(d)`，`Math::BigInt` 保存当前阶乘结果，其他辅助变量占用常数空间。

## 常见易错点

1. 负数必须输出 `Invalid input`，不能直接进入累乘循环。
2. `0!` 等于 1，结果应从 1 初始化。
3. 1000! 超出普通整数范围，应使用 `Math::BigInt`，并以字符串形式输出。
4. 本题要求函数直接打印结果，不要擅自改成返回阶乘值。
